...
class VerificarLinkView(APIView):
    ...
    def get(self, request, token):
        try:
            ...
            return redirect("https://nextstop-app-u9cvd.ondigitalocean.app/nextstop-frontend2/?estado=expirado&mensaje=Token%20inválido%20o%20no%20encontrado")

        usuario = verificacion.usuario

        # Si el correo ya estaba verificado
        if usuario.email_verificado:
            return redirect(f"https://nextstop-app-u9cvd.ondigitalocean.app/nextstop-frontend2/?estado=exito&mensaje=El%20correo%20{usuario.email}%20ya%20estaba%20verificado")

        # Si está expirado
        if verificacion.expirado():
            verificacion.usado = True
            verificacion.save()
            return redirect("https://nextstop-app-u9cvd.ondigitalocean.app/nextstop-frontend2/?estado=expirado&mensaje=El%20enlace%20ha%20expirado%20o%20ya%20fue%20usado")
        ...
        return redirect(f"https://nextstop-app-u9cvd.ondigitalocean.app/nextstop-frontend2/?estado=exito&mensaje=El%20correo%20{usuario.email}%20ha%20sido%20verificado%20exitosamente")